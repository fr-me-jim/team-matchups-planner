...
import type { Team, IMatchUp } from "src/app/interfaces/app.interfaces";

function calcMatchSkillGap(firstTeam: Team, secondTeam: Team): number {
    const firstTeamSkillGap = firstTeam.reduce((acc, player) => acc + player.skillLevel, 0);
    const secondTeamSkillGap = secondTeam.reduce((acc, player) => acc + player.skillLevel, 0);
...
function teamMatchMaking(
    ...
    if (index === players.length) {
        ...
        if (firstTeam.length >= minPlayersPerTeam && secondTeam.length >= minPlayersPerTeam) {
            matchups.push({ 
